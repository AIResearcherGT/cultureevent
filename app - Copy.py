...
# Specify the path to the new SQLite database file
db_path = 'new_events.db'
...
def dashboard():
    if request.method == 'POST':
        ...
        existing_event = Event.query.filter_by(title=title, start_date=start_date, end_date=end_date).first()
        if existing_event:
    ...
    organization = Organization.query.get(current_user.organization_id)  # Retrieve the organization
    return render_template('dashboard.html', events=all_events, csrf_token=csrf_token, organization=organization)
...
def get_events():
    all_events = Event.query.all()
    events_list = [e.serialize for e in all_events]
...
def user_favorites(user_id):
    ...
    events_data = [event.serialize for event in favorites]
    return jsonify(events_data), 200
